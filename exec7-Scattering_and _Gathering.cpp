...
int main(int argc, char **argv) {
  ...
  // Process 0 allocates and initializes the points
  if (rank == 0) {
  ...
  double *localPoints = new double[npts * 2 / size];
  MPI_Scatter(points, npts * 2 / size, MPI_DOUBLE, localPoints, npts * 2 / size, MPI_DOUBLE, 0, MPI_COMM_WORLD);

  // Computing the mandelbrot set.
  // This function is already coded and you don't have to worry about it
  int *mset = new int[npts / size];
  compute_mandelbrot(localPoints, npts / size, mset);

  // Gather results to process 0
  int *gatheredMset = nullptr;
  if (rank == 0) {
    gatheredMset = new int[npts];
  }

  MPI_Gather(mset, npts / size, MPI_INT, gatheredMset, npts / size, MPI_INT, 0, MPI_COMM_WORLD);

  // Printing results on process 0
  if (rank == 0) {
    for (int yp = 0; yp < p_count; ++yp) {
      for (int xp = 0; xp < p_count; ++xp)
        std::cout << gatheredMset[yp * p_count + xp] << " ";
      std::cout << std::endl;
    }
  }
